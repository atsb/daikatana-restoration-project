...
// load it in
	if (s->name[0] == '#')
		...
		Com_sprintf (namebuffer, sizeof(namebuffer), "sounds/%s", s->name);
//	Com_Printf ("loading %s\n",namebuffer);

	//	NELNO:	Hack until new sound stuff is checked in
	//			don't load sounds larger than 512 kb
	len = FS_FOpenFile (namebuffer, &f);
	if (len >= 0)
		FS_FCloseFile (f);
