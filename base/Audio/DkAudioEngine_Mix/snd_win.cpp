...
#include	"..\base\client\snd_loc.h"
#include	"winquake.h"
#include "..\audio\AudioEngineImports.h"

//	Nelno:	added for dk_Initializing variable
#include	"..\base\dk_\daikatana.h"
...
sndinitstat SNDDMA_InitDirect (void)
{
	...
	if ( !hInstDS )
	{
		...
		Com_DPrintf ("ok\n");
		*(void **)&pDirectSoundCreate = (void *)GetProcAddress(hInstDS,"DirectSoundCreate");
	...
	while ( ( hresult = iDirectSoundCreate( NULL, &pDS, NULL ) ) != DS_OK )
	{
		...
		if (dont_care_about_sound->value == 1.0f || MessageBox (NULL,
...
int SNDDMA_Init(void)
{
    ...
    dont_care_about_sound = Cvar_Get("dont_car_about_sound", "0", CVAR_ARCHIVE);
...
void SNDDMA_Submit(void)
{
	...
	while (1)
	{
		//	Nelno:	added !cls.disable_screen check
		//	this may cause screwy stuff to happen between level loads when a sound is playing...
//		if ( snd_completed == snd_sent && !dk_Initializing && !cls.disable_screen)
...
===========
*/
DLLEXPORT void S_Activate (qboolean active)
{
	if ( active )
	{
...
DLLEXPORT void S_ReverbPreset (int nPresetID)
{
}
