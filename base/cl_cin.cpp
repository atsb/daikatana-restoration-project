...
void SCR_LoadPCX (char *filename, byte **pic, byte **palette, int *width, int *height)
{
	...
	*pic = NULL;
	...
	out = (byte *)X_Malloc ( (pcx->ymax+1) * (pcx->xmax+1), MEM_TAG_CIN);
	...
	*pic = out;
	...
	if (palette)
	{
		*palette = (byte *)X_Malloc(768, MEM_TAG_CIN);
...
void Huff1TableInit (void)
{
	...
	cin.hnodes1 = (int *)X_Malloc (256*256*2*4, MEM_TAG_CIN);
...
//int		i;
	...
	input = in.data + 4;
	out_p = out.data = (byte *)X_Malloc (count, MEM_TAG_CIN);
...
qboolean SCR_DrawCinematic (void)
{
	...
	if (!cl.cinematicpalette_active)
	{
		re.CinematicSetPalette((const byte *)cl.cinematicpalette);
