...
//-----------------------------------------------------------------------------
// Name: DrawStretchPic32
// Desc: 32 bit version of DrawStretchPic
//-----------------------------------------------------------------------------
void DrawStretchPic32( const CPic *bitmap, int destx, int desty, int dest_width, int dest_height )
{
	...
	uint32		*dst_pixels	= (uint32 *) global_lock + (desty * display_pitch) + destx;
	...
	for( int row = 0; row < draw_height; row++, dst_pixels += display_pitch )
	{
		...
		if( bitmap_width == draw_width )
		{
			...
			{	
				dst_pixels[col] = palette->colors[src_pixels[col]].color32;
			}
		}
		else
		{
			...
			{
				dst_pixels[u]	= palette->colors[src_pixels[f>>16]].color32;
				f += fstep;

				dst_pixels[u+1]	= palette->colors[src_pixels[f>>16]].color32;
				f += fstep;

				dst_pixels[u+2]	= palette->colors[src_pixels[f>>16]].color32;
				f += fstep;

				dst_pixels[u+3]	= palette->colors[src_pixels[f>>16]].color32;
				f += fstep;
			}
		}
	}
...
//-----------------------------------------------------------------------------
// Name: DrawPic32
// Desc: 32 bit version of DrawPic
//-----------------------------------------------------------------------------
void DrawPic32(const CPic *bitmap, int destx, int desty, RECT *sourceRect )
{
	// make sure the surface pointer is valid
	if( global_lock == NULL )
		return;
