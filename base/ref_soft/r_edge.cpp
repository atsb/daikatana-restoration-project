...
extern "C" espan_t	*span_p, *max_span_p;
espan_t	*span_p, *max_span_p;

int		r_currentkey;

extern "C" int	current_iv;
int	current_iv;

extern "C" int	edge_head_u_shift20, edge_tail_u_shift20;
...
void D_DrawSpans16s (espan_t *pspan);
...
CVector			transformed_modelorg;
CVector			world_transformed_modelorg;
CVector			local_modelorg;
...
*/
	for (span=surf->spans ; span ; span=span->pnext)
	{
		pdest = (uint16 *)((byte *)d_viewbuffer + (r_screenwidth*span->v) + span->u * BYTES_PER_PIXEL);
		for ( u = 0; u <= span->count - 1 ; u++)
			pdest[u] = d_8to16table[color];
	}
}
...
void D_CalcGradients (msurface_t *pface)
{
	...
	TransformVector (pface->texinfo->s, p_saxis);
	TransformVector (pface->texinfo->t, p_taxis);
	...
	sadjust = ((fixed16_t)(DotProduct (p_temp1, p_saxis) * 0x10000 + 0.5)) -
			...
			+ pface->texinfo->s_offset*t;
	tadjust = ((fixed16_t)(DotProduct (p_temp1, p_taxis) * 0x10000 + 0.5)) -
			((pface->texturemins[1] << 16) >> miplevel)
			+ pface->texinfo->t_offset*t;
...
void D_TurbulentSurf (surf_t *s)
{
	...
	pface = s->msurf;
	miplevel = 0;
	cacheblock = (uint16*)pface->texinfo->image->pixels[0];
	cachewidth = 64;
	current_image_palette = (uint16*) pface->texinfo->image->palette;
...
//			TransformVector( pface->plane->normal, normal);
		...
		miplevel = D_MipLevelForScale(s->nearzi * scalefor_mip * pface->texinfo->mipadjust);
	}
#endif
...
//	cacheblock = (pixel_t *)pcurrentcache->data;
	cacheblock = (uint16 *)pcurrentcache->data;
...
void D_DrawflatSurfaces (void)
{
	...
	for (s = &surfaces[1] ; s<surface_p ; s++)
	{
		...
		// 16 bits of the msurface pointer
