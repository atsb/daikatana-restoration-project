...
*/
	...
	__asm mov bx,   word ptr [edi]
	__asm and ebx, r_blendmask50
	__asm and eax, r_blendmask50
	__asm add eax, ebx
...
{
	static CVector	local, transformed;
	static float	zi;
	static int      u, v, tmp;
	static short    izi;
	static int      ebpsave;
	...
	*/
	// local = pparticle->origin - r_origin;
...
** and goes.
*/
extern "C" unsigned long fpu_sp24_cw, fpu_chop_cw;
void R_DrawParticles (void)
{
	...
	for (p=r_newrefdef.particles, i=0 ; i<r_newrefdef.num_particles ; i++,p++)
	{
		...
		r = Q_ftol(p->color_rgb.x) >> sw_state.rgb16.remove.rgbRed;
		g = Q_ftol(p->color_rgb.y) >> sw_state.rgb16.remove.rgbGreen;
		b = Q_ftol(p->color_rgb.z) >> sw_state.rgb16.remove.rgbBlue;

		partparms.particle = p;
//		partparms.color    = p->color;
		partparms.color    = (r << sw_state.rgb16.position.rgbRed) |
...
void R_DrawComParticles (void)
{
	...
	for (p=r_newrefdef.comparticles, i=0 ; i<r_newrefdef.num_comparticles ; i++,p++)
	{
