...
extern "C" unsigned int	cacheoffset;
...
extern "C" medge_t			*r_pedge;
medge_t			*r_pedge;

extern "C" qboolean		r_leftclipped, r_rightclipped;
qboolean		r_leftclipped, r_rightclipped;
static qboolean	makeleftedge, makerightedge;
extern "C" qboolean		r_nearzionly;
qboolean		r_nearzionly;

//int		sintable[1280];
//int		intsintable[1280];
int		sintable[2048];
int		intsintable[2048];

extern "C" mvertex_t	r_leftenter, r_leftexit;
extern "C" mvertex_t	r_rightenter, r_rightexit;
...
extern "C" int				r_emitted;
extern "C" float			r_nearzi;
extern "C" float			r_u1, r_v1, r_lzi1;
extern "C" int				r_ceilv1;
...
extern "C" qboolean		r_lastvertvalid;
...
CVector	box_vecs[6][2] = {
	{	CVector(0,-1,0), CVector(-1,0,0) },
	{	CVector(0,1,0),  CVector(0,0,-1) },
	{	CVector(0,-1,0), CVector(1,0,0) },
	{	CVector(1,0,0),  CVector(0,0,-1) },
	{	CVector(0,-1,0), CVector(0,0,-1) },
	{	CVector(-1,0,0), CVector(0,0,-1) }
};
...
void R_InitSkyBox (void)
{
	...
	for (i=0 ; i<6 ; i++)
	{
		...
		r_skytexinfo[i].s = box_vecs[i][0];
		r_skytexinfo[i].t = box_vecs[i][1];
...
void R_EmitSkyBox (void)
{
	...
	for (i=0 ; i<6 ; i++)
	{
		r_skytexinfo[i].s_offset = -DotProduct (r_origin, r_skytexinfo[i].s);
		r_skytexinfo[i].t_offset = -DotProduct (r_origin, r_skytexinfo[i].t);
	}
...
void R_RenderFace (msurface_t *fa, int clipflags)
{
	...
	if (fa->texinfo->flags & (SURF_TRANS33|SURF_TRANS66|SURF_MIDTEXTURE))
...
void R_RenderBmodelFace (bedge_t *pedges, msurface_t *psurf)
{
	...
	if (psurf->texinfo->flags & (SURF_TRANS33|SURF_TRANS66|SURF_MIDTEXTURE))
