...
static int CDAudio_GetAudioDiskInfo(void)
{
	...
	// get tracks
	tracks = AIL_redbook_tracks(device);
	if (tracks < 1)
	{
...
void CDAudio_Play2(int track, qboolean looping)
{
	...
	// get track timings
	AIL_redbook_track_info(device,track,&startms,&endms);
	if (startms == 0 && endms == 0)
	{
		Com_Printf("CDAudio: track %d is not audio\n",track);
		return;
	}
	...
	// send play command
	if (AIL_redbook_play(device,startms,endms) == REDBOOK_ERROR)
	{
		Com_DPrintf("CDAudio: Play failed\n");
		return;
	}
...
void CDAudio_Stop(void)
{
	...
	if (AIL_redbook_stop(device) == REDBOOK_ERROR)
		Com_DPrintf("Stop failed\n");
...
void CDAudio_Pause(void)
{
	...
	if (AIL_redbook_pause(device) == REDBOOK_ERROR)
		Com_DPrintf("Pause failed\n");
...
void CDAudio_Resume(void)
{
	...
	if (AIL_redbook_resume(device) == REDBOOK_ERROR)
	{
		Com_DPrintf("CDAudio: Play failed\n");
		return;
	}
...
static void CD_f (void)
{
	...
	{
		CDAudio_Play(atoi(GetArgv (2)), false);
		return;
	}
	if (Q_strcasecmp(command, "loop") == 0)
	{
		CDAudio_Play(atoi(GetArgv (2)), true);
		return;
	}
...
{
	if ((cd_nocd->value?true:false) != !enabled)
	{
		if (cd_nocd->value)
		{
			CDAudio_Stop();
...
int CDAudio_Init(void)
{
	...
	{
		AIL_shutdown();
		Com_Printf("Audio system has not been started.\n");
		return -1;
	}

	// open cd device
	device = AIL_redbook_open(cd_device->value);
	if (!device)
	{
		Com_Printf("CDAudio_Init: Failed to Open CD Device\n");
		return -1;
	}
