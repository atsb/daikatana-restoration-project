...
// for debugging
long		out_bytes;		// out bytes
long		in_bytes;		// in bytes
long		last_time;

loopback_t	loopbacks[2];
int			ip_sockets[2];
#pragma message( "// SCG[1/22/00]: Why was IPX disabled?!?!? ")
// int			ipx_sockets[2];
...
void NET_SendPacket (netsrc_t sock, int length, void *data, netadr_t to)
{
	...
	if (net_debug->value)
	{
		...
		long diff = Sys_Milliseconds() - last_time;
		if (diff > 1000)
		{
			Com_Printf("IN: %d  OUT: %d\n", in_bytes/(diff/1000), out_bytes/(diff/1000));
...
void NET_Init (void)
{
	...
	net_debug = Cvar_Get("n_speeds","0",0);

	// setup for debuggin
	out_bytes = 0;		// out bytes
	in_bytes = 0;		// in bytes
	last_time = Sys_Milliseconds();
}
/*
