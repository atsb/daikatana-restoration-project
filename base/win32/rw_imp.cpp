...
//	HRESULT	ddrval;
	...
	if( !vid_fullscreen->value ){
		GetClientRect( sww_state.hWnd, &sww_state.screen_rect );
...
void RImp_EndFrame (void)
{
	...
	{
		if ( ( rval = sww_state.lpddsBackBuffer->BltFast( 0, 0, sww_state.lpddsOffScreenBuffer, &r, 
																DDBLTFAST_WAIT ) ) == DDERR_SURFACELOST )
		{
			sww_state.lpddsBackBuffer->Restore();
			sww_state.lpddsBackBuffer->BltFast( 0, 0, sww_state.lpddsOffScreenBuffer, &r, DDBLTFAST_WAIT );
		}
	...
	{
		sww_state.lpddsFrontBuffer->Blt( &sww_state.screen_rect, 
			sww_state.lpddsOffScreenBuffer, &sww_state.view_rect, DDBLT_WAIT, NULL );
	}

	memset( &ddsd, 0, sizeof( ddsd ) );
	ddsd.dwSize = sizeof( ddsd );

	if( FAILED( sww_state.lpddsOffScreenBuffer->Lock( NULL, &ddsd, DDLOCK_WAIT, NULL ) ) )
		ri.Sys_Error (ERR_FATAL, "Couldn't get surface pointer");

	vid.buffer = (byte*) ddsd.lpSurface;
	vid.rowbytes = ddsd.lPitch;
	vid.pixelwidth = vid.rowbytes / (vid.colordepth >> 3);
}
/*
** RImp_SetMode
*/
rserr_t RImp_SetMode( int *pwidth, int *pheight, int mode, qboolean fullscreen )
{
	...
	if ( fullscreen )
	{
		if ( !RImp_InitGraphics( 1 ) )
		{
			if ( RImp_InitGraphics( 0 ) )
			{
...
#else
extern "C" unsigned fpu_ceil_cw, fpu_chop_cw, fpu_full_cw, fpu_cw, fpu_pushed_cw;
extern "C" unsigned fpu_sp24_cw, fpu_sp24_ceil_cw;
