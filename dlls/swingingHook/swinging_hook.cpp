...
void hook_Drop (userEntity_t *ent_grapple)
{
   int i;
	...
	if(!ent_grapple)
	{
		g_server->Con_Dprintf ("hook_Drop():%s,%i: ent_grapple == NULL\n", __FILE__, __LINE__);
		return; 
	}
   ...
   {
      g_server->Con_Dprintf("hook_Drop():%s:%i: ent_grapple->goalentity == NULL\n", __FILE__, __LINE__);
   } else {
   ...
   ent_grapple->remove(ent_grapple);
	g_server->RemoveEntity(ent_grapple);
}
...
void hook_Behavior(userEntity_t *grapple)
{
   ...
   CVector  novector(0,0,0);
	...
	if(!grapple)
	{
		g_server->Con_Dprintf ("hook_Behavior():%s,%i: grapple == NULL\n", __FILE__, __LINE__);
		return; 
	}
	...
	if ( (!(grapple->owner->client->hookstate & HOOK_ACTIVE)) ||// if hook has been retracted
		 ...
		 ( (g_server->time - 5 > grapple->health) && (grapple->owner->client->hookstate & HOOK_ATTACKING) )  // if it's a noose/crucifier, it's got 5 seconds
		)	
	{
		hook_Drop(grapple);
		return;
...
//	AngleVectors (grapple->owner->client->v_angle, forward, right, NULL);
   ...
   chainvec.Subtract(grapple->s.origin, start);
	...
	grapple->nextthink = g_server->time + 0.1;
}
...
void hook_Attack(userEntity_t *attacking_hook, userEntity_t *victim)
{
   ...
   // Determine if the victim can hang
	// trace=g_server->trace_q2(victim->s.origin, NULL, NULL, traceup, victim, MASK_SOLID);
	trace=g_server->TraceLine_q2(victim->s.origin, traceup, NULL, MASK_SOLID);
...
void hook_Touch (userEntity_t *grapple_projectile, userEntity_t *other, cplane_t *plane, csurface_t *surf)
{
   ...
   CVector  novector(0,0,0);
	...
	{
		g_server->Con_Dprintf ("hook_Touch():%s,%i: grapple_projectile == NULL\n", __FILE__, __LINE__);
		return; 
	}
	if(!grapple_projectile->owner)
	{
		g_server->Con_Dprintf ("hook_Touch():%s,%i: grapple_projectile->owner == NULL\n", __FILE__, __LINE__);
		return; 
	}
	if(!grapple_projectile->owner->client)
	{
		g_server->Con_Dprintf ("hook_Touch():%s,%i: grapple_projectile->owner->client == NULL\n", __FILE__, __LINE__);
		return; 
	}
	if(!other)
	{
		g_server->Con_Dprintf ("hook_Touch():%s,%i: other == NULL\n", __FILE__, __LINE__);
		return; 
	}
	if(!plane)
	{
		g_server->Con_Dprintf ("hook_Touch():%s,%i: plane == NULL\n", __FILE__, __LINE__);
		return; 
	}
	if(!surf)
	{
		g_server->Con_Dprintf ("hook_Touch():%s,%i: surf == NULL\n", __FILE__, __LINE__);
		return; 
	}
   ...
   grapple_projectile->client->hookLength = chainvec.Length();	
	...
	if (other->solid == SOLID_BBOX)
	{
		if ((other->svflags & SVF_MONSTER) || (other->client) || other->flags & FL_BOT)
			g_server->sound (grapple_projectile, CHAN_AUTO, "e3/we_bolterhitmetal.wav", 1, ATTN_NORM);
	...
	if (other->solid == SOLID_BSP)
	{
		...
		g_server->WriteByte (SVC_TEMP_ENTITY);
		g_server->WriteByte (TE_SMOKE);
		g_server->WritePosition (grapple_projectile->s.origin);
		g_server->WriteFloat(1.0);
		g_server->WriteFloat(2.0);
		g_server->WriteFloat(2.0);
		g_server->WriteFloat(5.0);
		g_server->MultiCast (grapple_projectile->s.origin, MULTICAST_PVS);
   ...
   grapple_projectile->velocity = other->velocity;
	...
	grapple_projectile->nextthink = g_server->time + 0.1;
}
...
void hook_Fly (userEntity_t *ent)
{
	...
	if(!ent)
	{
		g_server->Con_Dprintf ("hook_Fly():%s,%i: ent == NULL\n", __FILE__, __LINE__);
		return; 
	}
	// ent->owner info is passed down the execution path
	if(!ent->owner || !ent->client)
	{
		g_server->Con_Dprintf ("hook_Fly():%s,%i: ent->owner || ent->client == NULL\n", __FILE__, __LINE__);
		return; 
	}
	// owner sure as hell better be a client!
	if(!ent->owner->client)
	{
		g_server->Con_Dprintf ("hook_Fly():%s,%i: ent->owner->client == NULL\n", __FILE__, __LINE__);
		return; 
	}
   ...
   chainvec = ent->s.origin - ent->owner->s.origin;
	...
	ent->nextthink = g_server->time + 0.1;
}
...
void hook_Fire (userEntity_t *ent)
{
   ...
   newhook->s.maxs.Zero();
	...
	newhook->s.modelindex = g_server->ModelIndex ("models/e1/we_swrocket.dkm");
	newhook->owner = ent;

	newhook->touch = hook_Touch;
	newhook->think = hook_Fly;
	newhook->nextthink = g_server->time + 0.1;
	
	g_server->LinkEntity (newhook);
	g_server->sound (newhook, CHAN_AUTO, "e2/we_discshoota.wav", 1, ATTN_NORM);
   ...
   // the chain between the client and the hook itself
	hookchain = g_server->SpawnEntity();
   ...
   hookchain->s.effects = EF_GIB; // not really, just force the ent to the client
	...
	hookchain->s.skinnum = GC_WHITE;
	hookchain->s.alpha = 1;
	hookchain->owner = ent;
   
   // hookchain->s.modelindex = 1;
	hookchain->s.modelindex = 0;//  g_server->ModelIndex ("models/e1/we_swrocket.dkm");
   // VectorCopy(newhook->s.origin, hookchain->s.origin);
   hookchain->s.origin = newhook->s.origin;
	g_server->LinkEntity (hookchain);
   newhook->goalentity = hookchain;
	...
	tinfo.modelindex = g_server->ModelIndex ("models/e1/we_swrocket.dkm");
   tinfo.fru.x = 0;
   tinfo.fru.y = -8;
   tinfo.fru.z = -8;
...
void grapple_Cmd(edict_s *ent)
{
	...
	if(!ent)
	{
		g_server->Con_Dprintf ("grapple_Cmd:%s,%i: ent == NULL\n", __FILE__, __LINE__);
		return; 
	}
	if(!ent->client)
	{
		g_server->Con_Dprintf ("grapple_Cmd:%s,%i: ent->client == NULL\n", __FILE__, __LINE__);
		return; 
	}

	// parameters for grappling hook command
	s = g_server->GetArgv(1);
...
{
	g_server = state;
	hook_speed = g_server->cvar ("hook_speed", "1000", CVAR_ARCHIVE);
	hook_min_len = g_server->cvar ("hook_min_len", "40", CVAR_ARCHIVE);
	hook_max_len = g_server->cvar ("hook_max_len", "2000", CVAR_ARCHIVE);
	hook_rate = g_server->cvar ("hook_rate", "40", CVAR_ARCHIVE);
	hook_prediction = g_server->cvar ("hook_prediction", "0", CVAR_ARCHIVE);
   g_cvarLogServer = g_server->cvar("log_server", "", CVAR_ARCHIVE);
}

void dll_LevelLoad(void)
{

	g_server->Con_Printf ("* Loaded Grappling Hook DLL\n");
	g_server->AddCommand("grapple", grapple_Cmd);
}
...
{
   com = (common_export_t *)g_server->common_exports;
...
void hook_Noose(userEntity_t *attackinghook, userEntity_t *victim, CVector forward)
{
   ...
   newhook->s.maxs.Zero();
	...
	newhook->health = g_server->time;
	newhook->volume = g_server->time;
	newhook->touch = hook_Touch;
	newhook->think = hook_Fly;
	newhook->nextthink = g_server->time + 0.1;
   ...
   hookchain->s.effects = EF_GIB; // not really, just force the ent to the client
	...
	hookchain->s.frame = 1; 
	// hookchain->s.skinnum = 0xa1a2a3a4;
	hookchain->s.skinnum = GC_RED;
	hookchain->s.alpha = 1;
	hookchain->owner = victim;
   
   // hookchain->s.modelindex = 1;
	hookchain->s.modelindex = g_server->ModelIndex ("models/e1/we_swrocket.dkm");
   // VectorCopy(newhook->s.origin, hookchain->s.origin);
   hookchain->s.origin = newhook->s.origin;
