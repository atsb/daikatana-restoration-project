...
*   overkill.  For non-trivial classes (eg: IString in particular) this is
*   a terrible implementation.
*
*   The bottom line: we have two special routines for doing construction
*   and destruction of arrays of special elements - in particular IStrings.
...
/////////////////////////////////////////////////////////////////////////////
// Special implementations for IStrings
// it is faster to bit-wise copy a IString than to call an official
//   constructor - since an empty IString can be bit-wise copied

static inline void ConstructElement(IString* pNewData)
{
	memcpy(pNewData, &ionEmptyString, sizeof(IString));
}

static inline void DestructElement(IString* pOldData)
{
	pOldData->~IString();
}

static inline void CopyElement(IString* pSrc, IString* pDest)
{
	*pSrc = *pDest;
}
