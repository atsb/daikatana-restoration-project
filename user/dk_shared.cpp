...
const CVector	vec3_origin( 0, 0, 0 );
const CVector	vec3_identity( 1.0f, 1.0f, 1.0f );
...
int BoxOnPlaneSide2( CVector &eMins, CVector &eMaxs, struct cplane_s *p )
{
	CVector corner1, corner2;
	...
	float dist1 = DotProduct(p->normal, corner1) - p->dist;
	float dist2 = DotProduct(p->normal, corner2) - p->dist;
...
/*
	if (sides == 0)
	{
		Com_Printf ("BoxOnPlaneSides: assert, sides == 0!!\n");
	}
*/
	return sides;
}
...
int BoxOnPlaneSide( CVector &emins, CVector &emaxs, cplane_t *p )
{
	// cast to float points for faster access
	...
	if ( p->type < 3 )
	{
		if (p->dist <= eminsPt[p->type])
		{
			return 1;
		}
		if (p->dist >= emaxsPt[p->type])
		{
			return 2;
		}
		return 3;
...
//unix - added LittleShort etc.
#ifdef __BIG_ENDIAN__
short	LittleShort (short b)	{return ShortSwap(b);}
int		LittleLong (int b)		{return LongSwap(b);}
float	LittleFloat (float b)	{return FloatSwap(b);}
#else
...
/*
short	BigShort(short l)		{return ShortSwap(l);}
short	LittleShort(short l)	{return ShortNoSwap(l);}
int		BigLong (int l)			{return LongSwap(l);}
int		LittleLong (int l)		{return LongNoSwap(l);}
float	BigFloat (float l)		{return FloatSwap(l);}
float	LittleFloat (float l)	{return FloatNoSwap(l);}
*/
...
void StripInvalidChars(char * name, int max_len)
// strip invalid characters from a string and truncates
{
  char bogus_chars[] = { '%', '/', '\\' };
  char *n;
  int loop, i, len, slen;
  ...
  // JAR caution: overwrite could occure here
//  if (max_len > -1) 
//    name[max_len] = '\0'; // truncate
  if(max_len > slen)
      name[slen] = '\0';
  else
      name[max_len] = '\0';
...
#define SWAP_BACKSLASHES

void swap_backslashes(char * buf)
// swap out backslashes with forward slashes
{
#ifndef SWAP_BACKSLASHES
  return;
#endif
...
void VectorToAngles(vec3_t vec1, vec3_t angles)
{
	float	forward;
	float	yaw, pitch;
	
	if (vec1[1] == 0 && vec1[0] == 0)
	{
		yaw = 0;
		if (vec1[2] > 0)
			pitch = 90;
	...
	{
		yaw = (int) (atan2(vec1[1], vec1[0]) * 180 / M_PI);
		if (yaw < 0)
			yaw += 360;

		forward = sqrt (vec1[0] * vec1[0] + vec1[1] * vec1[1]);
		pitch = (int) (atan2 (vec1[2], forward) * 180 / M_PI);
...
{
	mins.Set( 99999, 99999, 99999 );
	maxs.Set( -99999, -99999, -99999 );
}
